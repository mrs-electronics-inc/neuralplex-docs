...
2. Pair and Connect from Your Phone:
    ...
    On your phone, enable Bluetooth, search for devices, and pair with the NeuralPlex (it should appear as "neuralplex").
    While `bluetoothctl` is open, you will need to type `yes` to authorize multiple services (audio and phone calls) during the pairing process.
    Once paired, connect the phone to the NeuralPlex as an audio device (A2DP profile). Your phone should show it as connected for media audio.
...
4. Play Audio
    ...
    ```bash title="Enable Bluetooth Audio"
